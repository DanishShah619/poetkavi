...
const MOOD_KEYWORDS = [
  { id: "melancholic", label: "Melancholic", marker: "M" },
  { id: "self-musing", label: "Self-Musing", marker: "S" },
  { id: "romantic", label: "Romantic", marker: "R" },
  { id: "betrayed-passion", label: "Betrayed Passion", marker: "B" },
  { id: "lively", label: "Lively", marker: "L" },
  { id: "uplifting", label: "Uplifting", marker: "U" },
  { id: "reflection", label: "Reflection", marker: "Re" },
];

const STYLE_KEYWORDS = [
  { id: "sufi", label: "Sufi", marker: "Su" },
  { id: "rich-imagery", label: "Rich Imagery", marker: "I" },
  { id: "persian", label: "Persian", marker: "P" },
  { id: "arabic", label: "Arabic", marker: "A" },
  { id: "ornate", label: "Ornate", marker: "O" },
  { id: "simple", label: "Simple", marker: "Si" },
  { id: "dramatic", label: "Dramatic", marker: "D" },
];
...
function KeywordChip({
  ...
  return (
    <motion.button
      ...
      `}
    >
      <span className="text-[10px] font-bold uppercase text-current/80">{marker}</span>
      <span>{label}</span>
...
export function AIAssistDialog({
  ...
  const [assistMode, setAssistMode] = useState<AssistMode>("continue");
  ...
  const toggleKeyword = (id: string) => {
    setSelected((prev) => {
      if (prev.includes(id)) return prev.filter((keyword) => keyword !== id);
      if (prev.length >= MAX_KEYWORDS) return prev;
      return [...prev, id];
    });
  };

  const resetSuggestion = (mode: AssistMode) => {
    setAssistMode(mode);
    setSuggestion(null);
  ...
  const handleGenerate = async () => {
    ...
    if (assistMode === "continue" && !poemContent.trim()) {
      setError("Write a few lines before asking AI to continue your poem.");
      return;
    }
    ...
    const labelMap = [...MOOD_KEYWORDS, ...STYLE_KEYWORDS].reduce<Record<string, string>>(
      (acc, keyword) => {
        acc[keyword.id] = keyword.label;
        return acc;
      },
      {}
    );
    const keywordLabels = selected.map((id) => labelMap[id]).filter(Boolean);
    ...
    try {
      ...
      const res = await fetch("/api/ai-assist", {
        ...
        body: JSON.stringify({ poemContent, keywords: keywordLabels, assistMode }),
      });
  ...
  const hasPoemContent = poemContent.trim().length > 0;
          ...
          >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-neutral-500">
                    Powered by Gemini -{" "}
                    {remaining !== null ? `${remaining} uses left this hour` : `${MAX_KEYWORDS} keywords max`}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 rounded-xl border border-neutral-800 bg-neutral-950 p-1">
                  <button
                    type="button"
                    onClick={() => resetSuggestion("continue")}
                  ...
                  <button
                    type="button"
                    onClick={() => resetSuggestion("rewrite")}
                    className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${
                      assistMode === "rewrite"
                        ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20"
                        : "text-neutral-400 hover:bg-neutral-900 hover:text-white"
                ...
                <div className="flex flex-wrap gap-2">
                  {MOOD_KEYWORDS.map((keyword) => (
                    <KeywordChip
                      ...
                      disabled={!selected.includes(keyword.id) && selected.length >= MAX_KEYWORDS}
              ...
              {hasPoemContent ? (
                <p className="text-[11px] text-neutral-500 bg-neutral-900 rounded-lg px-3 py-2 border border-neutral-800">
                  {assistMode === "continue"
                    ? "AI will suggest only the next lines for your poem."
                    : "AI will rewrite your existing poem using the selected keywords."}
                </p>
              ) : (
                <p className="text-[11px] text-amber-500/80 bg-amber-950/20 rounded-lg px-3 py-2 border border-amber-800/30">
                  Add a few lines to use Continue, or switch to Rewrite to compose a new poem.
                </p>
              )}
              ...
              {suggestion && (
                ...
                >
                  <p className="text-[11px] font-semibold text-emerald-400 uppercase tracking-widest flex items-center gap-1.5">
                    ...
                    {assistMode === "continue" ? "AI Continuation" : "AI Rewrite"}
                  </p>
              ...
              {confirmOverwrite ? (
                ...
                >
                  <p className="text-xs text-amber-400 text-center font-semibold mb-2">
                    You have existing poem content. What would you like to do?
                  </p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <CheckCheck className="w-3.5 h-3.5" />
                    {assistMode === "continue" ? "Use Continuation" : "Use Rewrite"}
                  </button>
                </div>
              ) : (
                <button
